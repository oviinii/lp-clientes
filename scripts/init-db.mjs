...
const email=process.env.ADMIN_EMAIL||'admin@lpclientes.local'
const password=process.env.ADMIN_PASSWORD||'admin123'
const existing=await db.execute({sql:'SELECT id FROM admins WHERE email=?',args:[email]})
if(existing.rows.length===0)await db.execute({sql:'INSERT INTO admins(name,email,password_hash) VALUES(?,?,?)',args:['Admin',email,await bcrypt.hash(password,10)]})
console.log(`Database ready at ${url}. Admin: ${email}`)
